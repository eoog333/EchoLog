...
모든 명령어를 실행하셨다면, 터미널에서 브라우저 창이 자동으로 열리며 **EchoLog** 화면이 나타납니다.

1. **이야기 시작하기:** 화면 중앙의 **'마이크 아이콘'**을 누르고, 오늘 하루 있었던 일을 편하게 이야기해 보세요.
2. **파일 업로드하기:** 이미 녹음해 둔 음성 파일이 있다면, 화면의 '업로드' 버튼을 눌러 테스트해 볼 수 있습니다.
3. **결과 확인하기:** 처리가 끝나면, 복잡했던 이야기가 **시간 흐름에 따라 깔끔하게 정돈된 타임라인**으로 나타나는 것을 확인하실 수 있습니다!

> 💡 **참고:** 혹시 창이 자동으로 열리지 않았다면 [http://localhost:5173](http://localhost:5173) 으로 직접 접속해 주세요.
### 환경변수 (`.env`)
