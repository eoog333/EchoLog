...
UI는 기능 확인 수준으로 유지하고, 현재 버전은 Timeline 모드로 동작합니다.
LLM 기반 서술체 변환은 향후 확장 항목입니다.
...
> Markdown 저장은 현재 범위에 포함하지 않으며 향후 확장 항목으로 둡니다.
...
**설계 원칙:** RTZR STT와 후처리 파이프라인이 프로젝트의 중심이며, LLM 기반 문체 변환은 향후 선택적으로 확장합니다.
...
[RTZR 응답 utterances]
        ↓
1. parse_utterances()     — utterance → 문단 리스트로 파싱
        ↓
2. sort_by_time()         — start_at 기준 시간순 정렬
        ↓
3. remove_duplicates()    — 인접한 유사 문장 제거 (임계값 0.8, 최대 간격 3초)
        ↓
4. group_into_events()    — 시간 간격 기준 사건 단위 그룹핑
        ↓
[ProcessedTranscript]     — 후처리 완료 결과
        ↓
5. Timeline Reflection 생성
        ↓
[Reflection]
```
...
| 중복 제거 | 짧은 시간 안에 연속으로 발생한 유사 발화 정리 |
| 사건 단위 정리 | 하루 동안 발생한 사건을 읽기 쉬운 형태로 재배열 |
| LLM | 향후 서술체 변환 기능으로 확장 예정 |

**현재 출력 예시 (Timeline 모드):**
...
| `get_result(transcribe_id)` | 전사 상태 및 결과 단건 조회 |
